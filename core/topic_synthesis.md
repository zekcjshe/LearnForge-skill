...
围绕一个**技术主题**（如 `[[TCP 拥塞控制]]`、`[[0-1 背包问题]]`、`[[RSA 签名]]`），将视频降级为“证据材料”，通过“任务自适应规划 (Task Profile)”、“缺口驱动检索”与“五级 Token 漏斗”，最终沉淀为一篇**权威、完整、自包含的主题知识对象**。
...
| 模式 | 触发场景 | 融合策略 |
|---|---|---|
| **1. 覆盖型 (Coverage)** | 视频 A 讲透了理论公式，缺少工程抓包；视频 B 补充了 Wireshark 抓包 | 依据 `task_profile.must_cover` 查缺补漏，将多源证据拼装入统一骨架 |
