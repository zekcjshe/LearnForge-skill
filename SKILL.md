...
### 3. 锚点真实性铁律（强制溯源，不可省略）
**引文层在万字笔记中占比天然很低（上限 6 条 × 100 字 = 600 字的绝对天花板），这是刻意设计而非执行不足**——锚点的力量来自稀缺，一周能记住几句就是几句，堆满引号反而是噪音。可读性由正文句式与讲师比喻承担，不由引号数量承担。

因此**锚点的质量维度只有一个：真实**。引文层一旦出现伪锚点，整篇笔记的溯源可信度归零，而学习者恰恰最信任带引号的话。铁律：

- **必须逐字出自原片字幕**。严禁把改写、转述、AI 润色后的句子放进 `> [!quote]` 块冒充原话——这是本规范最严重的违规。
- **必须带真实时间戳区间**，且与字幕中该句的实际位置一致。严禁使用 `[00:00 - 30:00]` 这类跨度极大的占位时间戳。
- **署名必须与真实信源一致**：UP 主名须与 frontmatter `sources[].author` 对应，杜绝同一文件内署名互相矛盾。
- **字幕为 AI 转写时**，引文可做同音错字正字化（如 `直换` → `置换`、`棘手` → `敌手`），但**不得改动语序与语义，也不得删改口语填词**（`呃`/`啊`/`呢`/`吧` 均为原话组成部分，删掉就匹配不上）。正字化后**仍须能在原字幕中按子串匹配到**；若某句的修正点正好落在引文中间，就**把引文截断在修正点之前**，不要为了句子完整而破坏可溯源性。
- **[强制] 质量门禁必须传 `--archive`**：不传时 `validate_note.py` 仅打印 warning 并**跳过锚点原文比对**，伪锚点无法被查出。传参后锚点会与原始字幕逐字核对（忽略标点差异），编造的句子当场暴露。
  ```bash
  # archive 为 JSON，须含 content / content_dehydrated / content_plain 三者之一（validate_note.py 只读这三个键）
  # ⚠️ 必须装「无时间戳的纯字幕」：带 [mm:ss --> mm:ss] 区间的文本不被 _TS_STRIP 识别，
  #    折叠后会残留 000211000212 这类数字串，把原本连续的句子切断，导致忠实引用被误判为编造。
  # ⚠️ archive 须覆盖笔记引用的【全部】信源，缺哪个视频，哪个视频的锚点就会误报。
